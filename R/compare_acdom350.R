...
## Both dataset are processed in process_cdom_asmala2014.R and
## process_cdom_massicotte.R
...
## Loop to create the linear models
...
p2 <- ggplot(df, aes(x = wl, y = slope)) +
  ...
  xlab("Wavelength (nm.)") +
  ylab(expression(paste("Slope (", m^{-1}, ")")))
...
p3 <- ggplot(df, aes(x = wl, y = intercept)) +
  ...
  geom_errorbar(aes(ymin = intercept - intercept_std, ymax = intercept + intercept_std),
                size = 0.1, col = "firebrick") +
  xlab("Wavelength (nm.)") +
  ylab(expression(paste("Intercept (", m^{-1}, ")")))
