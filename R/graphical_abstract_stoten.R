...
pA <- ggplot() +
  ...
  scale_y_continuous(sec.axis = sec_axis(~. * 27.64, name = bquote(a^"*"*~(m^2%*%mol~C^{-1}))), limits = c(NA, 6)) +
  annotate("text", -2200, 4.5, label = r2, vjust = 0, hjust = 0, parse = TRUE) +
  annotate(
    "text",
  ...
  labs(title = "DOM aromaticity along the aquatic continuum") +
  geom_segment(aes(x = max(res2$distance2), y = 5.9, xend = 0, yend = 5.9), size = 0.25, color = "#317D74") +
  geom_segment(aes(x = 0, y = 5.9, xend = min(res2$distance2), yend = 5.9), size = 0.25, color = "#3182bd") +
  geom_label(
    aes(
      x  = max(res2$distance2) / 2,
      y = 5.9,
      label = "Freshwater\necosystems"
    ),
    vjust = 0.5,
  ...
  geom_segment(aes(x = 0, y = 6, xend = 0, yend = 5.8), size = 0.25) +
  geom_segment(aes(x = max(res2$distance2), y = 6, xend = max(res2$distance2), yend = 5.8), size = 0.25, color = "#317D74") +
  geom_segment(aes(x = min(res2$distance2), y = 6, xend = min(res2$distance2), yend = 5.8), size = 0.25, color = "#3182bd") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())


ggsave("graphs/graphical_abstract_stoten.pdf", height = 4, width = 6)
embed_fonts("graphs/graphical_abstract_stoten.pdf")
