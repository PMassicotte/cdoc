...
test_that("Has appropriate numerical values", {
  ...
  # All DOC value should be greater than 0
  expect_true(all(unlist(lapply(data, function(x){x$doc})) > 0))
  
  # All cdom value should be greater than 0
  expect_true(all(unlist(lapply(data, function(x){x$acdom})) > 0))
})
